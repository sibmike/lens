...
**logit(q̂ᵢⱼ) = logit(qᵢ) + xᵢᵀβⱼ + εᵢⱼ**

Where:

* q̂ᵢⱼ = perceived quality of candidate i by evaluator j
* qᵢ = true quality of candidate i (unknown to evaluators)
...
function perceive_quality(true_prob, features, evaluator):
...
function generate_candidates(n_candidates):
 quality_tiers = {
 ...
 candidates = []
 for tier, (min_q, max_q) in quality_tiers.items():
 n_in_tier = n_candidates * tier_proportions[tier]
 qualities = random_uniform(min_q, max_q, size=n_in_tier)
...
class EvaluatorProfile:
 def __init__(self, role='investor'):
 if role == 'angel':
 self.pitch_sensitivity = clip(normal(0.75, 0.15), 0.3, 1.2)
 self.profile_bias = 0 if random() < 0.3 else normal(0.5, 0.3)
 self.noise_std = 0.5
 elif role == 'vc_partner':
 self.pitch_sensitivity = clip(normal(0.5, 0.1), 0.2, 0.8)
 self.profile_bias = normal(0.7, 0.2) # Stronger credential bias
 self.noise_std = 0.3 # More consistent
 elif role == 'ai_model':
 self.pitch_sensitivity = 0.0 # Unbiased
 self.profile_bias = 0.0
 self.noise_std = 0.2 # Lower noise than humans

**[Figure 3.2 — to be added: Evaluator Landscape.]**
...
function sequential_selection(candidates, evaluator, threshold, max_selections):
 selected = []
 for candidate in random_order(candidates):
 perceived_quality = evaluator.perceive(candidate)
 if perceived_quality >= threshold and len(selected) < max_selections:
...
function batch_selection(candidates, evaluator, selection_rate):
 ...
 n_select = int(len(candidates) * selection_rate)
 return [candidate for candidate, score in perceptions[:n_select]]
...
function committee_selection(candidates, committee_members, aggregation='mean'):
 candidate_scores = {}

 for candidate in candidates:
 scores = [member.perceive(candidate) for member in committee_members]

 if aggregation == 'mean':
 candidate_scores[candidate] = mean(scores)
 elif aggregation == 'median':
 candidate_scores[candidate] = median(scores)
 elif aggregation == 'max': # Optimistic
 candidate_scores[candidate] = max(scores)

 return select_top_k(candidate_scores, k=target_portfolio_size)
...
**[Figure 3.3 — to be added: Architecture Comparison.]**
...
stage_features = {
 'initial_screen': ['pitch_quality', 'elite_profile'],
 'deep_dive': ['pitch_quality', 'elite_profile', 'customer_traction', 'technical_validation'],
 'final_decision': ['all_features'] + ['reference_checks', 'background_investigation']
}
...
**Objective: max E[q | selected] = (1/k) Σᵢ₌₁ᵏ qᵢ**
...
**Objective: max P(at least one success) = 1 - Πᵢ₌₁ᵏ(1 - qᵢ)**
...
**Objective: min CV = σ(q_selected) / μ(q_selected)**
...
**Objective: max αE[q] + βP(success) - γCV - δCost**
...
function optimize_architecture(objectives, constraints):
 pareto_frontier = []

 for architecture in generate_architectures():
 if meets_constraints(architecture, constraints):
 performance = evaluate_objectives(architecture, objectives)
 if is_pareto_optimal(performance, pareto_frontier):
 pareto_frontier.append((architecture, performance))

 return pareto_frontier

**[Figure 3.4 — to be added: Objective Tradeoff Visualization.]**
...
for i in range(n_candidates):
 perceived[i] = logit_inverse(logit(true_prob[i]) + bias * features[i] + noise[i])

# Efficient: Vectorized operations
perceived = logit_inverse(logit(true_prob) + bias @ features.T + noise)
...
function monte_carlo_evaluation(architecture, n_simulations=1000):
 results = []

 for sim in range(n_simulations):
 # Generate random candidate pool
 candidates = generate_candidates(n=10000)

 # Create evaluators with random biases
 evaluators = [EvaluatorProfile() for _ in range(architecture.n_evaluators)]
 ...
 # Calculate metrics
 results.append({
 'avg_quality': mean(selected.true_quality),
 'portfolio_success': 1 - product(1 - selected.true_quality),
 'consistency': std(selected.true_quality) / mean(selected.true_quality)
 })
...
function adaptive_optimization(objective, parameter_space, budget=10000):
 # Initial exploration with Latin Hypercube
 initial_samples = latin_hypercube_sample(parameter_space, n=100)
 results = evaluate_batch(initial_samples, objective)

 # Build Gaussian Process model
 gp_model = fit_gaussian_process(initial_samples, results)

 # Adaptive sampling
 for iteration in range(budget - 100):
 # Find point with highest expected improvement
 next_point = maximize_expected_improvement(gp_model, parameter_space)

 # Evaluate and update model
 result = evaluate(next_point, objective)
 gp_model.update(next_point, result)

 return gp_model.find_optimum()

**[Figure 3.5 — to be added: Computational Scaling.]**
...
for iteration in 1 to 100:
 ...
 candidates = random_sample(population, size=review_count)

 # Generate evaluator profiles with random biases
 evaluators = create_evaluator_pool(archetype_parameters)

 # Run selection pipeline
 selected = pipeline.process(candidates, evaluators)

 # Calculate performance metrics
 record_metrics(selected)
...
* **Evaluator biases**: β_pitch ~ N(0.75, 0.15), β_profile ~ N(0.5, 0.3) with 30% showing no profile bias
...
**[Figure 4.1 — to be added: Simulation Flow Diagram.]**
...
archetype_parameters = {
 'review_count': [120 to 10000], # Deal flow volume
 'stages': [1 to 5], # Process complexity
 'evaluators_per_stage': [1 to 50], # Resource intensity
 'selection_mechanism': ['threshold', 'ranking', 'hybrid'],
 'aggregation_method': ['mean', 'median', 'max']
}

**[Figure 4.2 — to be added: Archetype Comparison Matrix.]**
...
function apply_pitch_filter(startups):
 for startup in startups:
 if startup.pitch_quality < 0:
 startup.pitch_quality = 0 # Bring up to minimum standard
...
**Portfolio Success Probability**: 1 - ∏(1 - p_i) for all portfolio companies. This captures the probability of achieving at least one major success—critical for fund-level returns.
...
metrics = {
 'quality': mean(selected.true_probability),
 'quality_std': std(selected.true_probability),
 'consistency_cv': quality_std / quality,
 'expected_unicorns': sum(selected.true_probability),
 'portfolio_success_prob': 1 - product(1 - selected.true_probability),
 'efficiency': expected_unicorns / portfolio_size,
 'acceptance_rate': portfolio_size / candidates_reviewed
}

**[Figure 4.3 — to be added: Metrics Visualization.]**
...
**[Figure 5.1 — to be added: Quality-Consistency Scatter Plot.]**
...
**[Figure 5.2 — to be added: Portfolio Success Probability Curves.]**
...
## 5.7 AI Filter Sensitivity (Paper 2 ↔ Paper 3 reconciliation)

The headline platform-vs-elite-VC comparison treats the AI-filter stage as effectively unbiased and low-noise. Paper 3 of this trilogy measures the corresponding real-world quantity: an LLM-based pitch ranker reaches NDCG@20 = 0.923 against a 10-expert ground truth. That measured ranker is not noiseless; mapped onto the log-odds noise scale of our perception model, its effective σ_AI is non-zero.

To check that the platform-architecture conclusion is not an artifact of AI infallibility, we sweep σ_AI across a range that brackets Paper 3's measured value and re-run the platform vs. elite-VC comparison at each point. The script is [`code/paper2_simulation/ai_sensitivity.py`](code/paper2_simulation/ai_sensitivity.py); the output is in [`code/paper2_simulation/results/sensitivity_curve.csv`](code/paper2_simulation/results/sensitivity_curve.csv) with the corresponding plot at [`code/paper2_simulation/results/sensitivity_curve.png`](code/paper2_simulation/results/sensitivity_curve.png).

**[Figure 5.7 — to be added: platform mean portfolio quality and elite-VC mean portfolio quality as a function of AI-filter noise σ_AI ∈ [0, 0.6], with the Paper 3 measured point marked at σ_AI ≈ 0.35.]**

The qualitative result is that the platform's quality is robust to AI noise within the realistic range: the platform's headline edge over solo angels and accelerators is driven primarily by *architecture* (multi-stage decomposition, batch evaluation, committee aggregation), not by the AI filter being perfect. The exact magnitude of the platform-vs-elite-VC gap does shift with σ_AI; the v2 paper's headline numbers should be read as the σ_AI = 0 limit of the curve. A separate calibration of σ_AI from the production pipeline (using [`code/data/ai_reviews_prod_anon.csv`](code/data/ai_reviews_prod_anon.csv)) is the natural next refinement.
...
**Sequential Selection Error**: ε_seq = σ²_threshold + σ²_perception + σ²_temporal

**Batch Selection Error**: ε_batch = σ²_perception / √n_compared

Where n_compared represents the number of simultaneous comparisons. For a batch of 100 startups, batch evaluation reduces error by approximately 10x through comparative calibration. This explains why top accelerators (batch evaluation) achieve 58% higher quality than solo angels (sequential) despite similar individual evaluator capabilities.
...
**Individual Variance**: Var(q̂_individual) = β²σ²_features + σ²_noise

**Committee Variance**: Var(q̂_committee) = β²σ²_features + σ²_noise/m
...
**Var(q̂_committee) = β²σ²_features + σ²_noise(1 + (m-1)ρ)/m**
...
**ΔQuality = (I₂/I_total) × log(n₁/n₂) × (1 - ρ_stages)**

Where n₁/n₂ represents the filtering ratio and ρ_stages captures information overlap. For typical parameters (70% new information, 20:1 filtering, 0.3 correlation), Stage 2 multiplies quality by 2.1x rather than adding a fixed increment.
...
**Quality = f(scrutiny) = f(total_resources / opportunities_reviewed)**
...
**Quality_multistage = Π f_i(resources_i / surviving_opportunities_i)**
...
1. **Efficient early filtering**: P(reject | truly_bad) > 0.9 with minimal resources
2. **Preserved option value**: P(reject | truly_good) < 0.1 at each stage
3. **Resource concentration**: resources_final / resources_initial > filtering_ratio
...
**P(miss) = P(A_misses) × P(B_misses|A_misses)**
...
1. **Repeated sampling**: Each startup gets evaluated by multiple scouts, reducing individual noise by √n_evaluations
...
The LENS framework was developed against venture-capital data, but every architectural mechanism we identify generalizes to other domains where multiple human evaluators select from large candidate pools under uncertainty. Three near-neighbor applications illustrate the transfer.

## 7.1 Hiring and Talent Selection

Corporate hiring pipelines exhibit the same architectural structure as VC: a large applicant pool reduced through sequential stages — recruiter screen, hiring-manager interview, panel interview, executive sign-off — each adding cost and information. The LENS predictions apply directly. **Homophily compounding** (Paper 1, Phenomenon 3) explains why pipelines that look diverse at the top of the funnel produce homogeneous hires: small per-stage biases (β_shared ≈ 0.2 in log-odds) accumulate multiplicatively across four interview stages, yielding 2–3× advantage for in-group candidates. **Committee paradox** (Paper 1, Phenomenon 5) explains why expanding hiring panels from 3 to 12 members fails to reduce systematic bias when panel members share recruiting backgrounds (ρ_β ≈ 0.7–0.9). The architectural prescription is identical to ours for VC: diversify evaluator *biases* (not just demographics), use batch evaluation where comparison is cheap, and front-load filters that operate on observable features distinct from those weighted by later stages.

## 7.2 Research Grant Review

Grant review panels face a tighter version of the same problem: smaller applicant pools but extreme selectivity (often <10% funding rates), correlated reviewer backgrounds (most reviewers come from the same field), and reviewer noise that meaningfully changes outcomes (inter-rater reliability of 0.259 in NIH studies). Our committee-aggregation result (Var(β̄) = σ²_β [ρ_β + (1 − ρ_β)/k]) implies that adding reviewers without diversifying biases buys precision at the cost of false confidence. The platform-architecture insight transfers: programs that decompose review into a low-cost screening stage (eligibility, scope, novelty) followed by deep review of survivors can match the quality of full-panel review at substantially lower expert cost — exactly the dynamic that drives the platform-vs-elite-VC gap in our simulations.

## 7.3 Academic Admissions

University admissions face the same multi-stage challenge with two domain-specific twists: **recommendation power** (Paper 1, Phenomenon 6) is more concentrated, since admissions officers explicitly weight letters of recommendation; and outcomes are weakly observable (graduation, post-graduate placement) on long timescales. The first twist amplifies the homogeneity risk we modeled in §5: when recommenders share educational backgrounds with admissions evaluators, the recommendation stage operates as a cost-free Stage 0 with ρ_β close to 1, propagating in-group preferences through the rest of the pipeline. The second twist — slow outcome observability — is exactly the regime where the consistency dimension we introduce matters most: institutions cannot tell within a single admissions cycle whether their selection is improving or drifting, so a process with low CV is more credible than one whose mean quality is slightly higher but whose year-to-year variance is wide.

## 7.4 What Does *Not* Generalize

The framework assumes (a) bounded probability outcomes, (b) multiple evaluators, (c) designable architecture, and (d) eventually observable outcomes for calibration. Domains that violate these — pure-preference selection (favorite color), single-evaluator decisions, fully constrained processes (regulated lottery allocation), or domains without ground truth — sit outside the LENS scope. The HDSE prescriptions in Paper 1 §7 ("When LENS May Fail") apply unchanged to these adjacent domains.
...
1. **Pitch sensitivity** (β_pitch): How much communication quality influences your perception
2. **Profile bias** (β_profile): How much credentials/background affect your judgment
...
The trilogy's full bibliography (covering operations research, behavioral economics, signal detection theory, and venture-capital empirical literature) is consolidated in Paper 1 (LENS) and Paper 3 (Instruction Distillation). The references most directly cited in this paper are:

1. F. Galton, "Vox populi," *Nature*, vol. 75, no. 1949, pp. 450–451, Mar. 1907.
2. C. Fernández-Aráoz, B. Groysberg, and N. Nohria, "The definitive guide to recruiting in good times and bad," *Harvard Business Review*, May 2009.
3. A. W. Brooks, L. Huang, S. W. Kearney, and F. E. Murray, "Investors prefer entrepreneurial ventures pitched by attractive men," *Proceedings of the National Academy of Sciences*, vol. 111, no. 12, pp. 4427–4431, 2014.
4. Cyrannus Inc. internal data; production AI review log and expert ratings, anonymized in [`code/data/`](code/data/).
5. Arbuzov, M. L., and Mosbacker, L. (2025). *LENS: A Mathematical Foundation for Human Decision-Systems Engineering.* Companion paper (Paper 1 of the LENS trilogy).
6. Arbuzov, M. L., and Mosbacker, L. (2025). *Instruction Distillation for Startup Pitch Ranking: LLMs as Scalable First-Pass Filter.* Companion paper (Paper 3 of the LENS trilogy).

The full reference list with secondary literature in operations research, behavioral economics, and venture-capital empirics will be expanded for the journal version.
