...
*Cyrannus Inc., 2025. Second paper in the LENS trilogy. Paper 1 develops the framework. This paper stress-tests it through Monte Carlo simulation across eleven investor archetypes and finds the central architectural result: under the model's assumed candidate-quality distribution and evaluator-parameter ranges, an angel investor accessing a well-designed platform outperforms elite venture firms by 11.4% on portfolio quality. Paper 3 instantiates the AI-filter component of that platform architecture in a working system.*
...
The headline platform-vs-elite-VC comparison treats the AI-filter stage as effectively unbiased and low-noise. Paper 3 of this trilogy measures the corresponding real-world quantity *in-sample on N = 35 pitches*: a large language model with carefully designed prompts ranks pitches in alignment with a ten-expert panel at NDCG@20 = 0.923. That measurement *motivates* a plausible range for $\sigma_{\mathrm{AI}}$ when mapped onto the log-odds noise scale of our perception model; it does not pin down a production estimate, since $N = 35$ is too small and the result is in-sample.

To avoid leaning on a single point estimate from Paper 3, we sweep $\sigma_{\mathrm{AI}}$ across a wide range — substantially broader than Paper 3's nominal value would suggest — and re-run the platform vs. elite-VC comparison at each point. The script is [`code/paper2_simulation/ai_sensitivity.py`](../code/paper2_simulation/ai_sensitivity.py); the output is in [`code/paper2_simulation/results/sensitivity_curve.csv`](../code/paper2_simulation/results/sensitivity_curve.csv). The qualitative result is that the platform's quality is robust to AI noise across the swept range. The platform's headline edge is driven primarily by the architecture (multi-stage decomposition, batch evaluation, committee aggregation), not by the AI filter being perfect. The exact magnitude of the platform-vs-elite-VC gap shifts with $\sigma_{\mathrm{AI}}$, but the qualitative ordering does not. The trilogy thus does not depend on Paper 3's small-N result being a stable production estimate; it depends on the architectural conclusion holding across a wide $\sigma_{\mathrm{AI}}$ range that brackets it.
...
The platform paradox is not really a paradox; it is what the LENS framework predicts. Paper 1 develops the perception model that makes the prediction. This paper stress-tests the prediction through simulation across eleven realistic investor archetypes. Paper 3 instantiates the AI-filter stage of the platform architecture in a working system that aligns with a ten-expert panel at NDCG@20 = 0.923 *in-sample*. Together, the trilogy moves the design of selection systems from craft toward engineering, with a coherent modeling framework, conditional simulation evidence, and an operationally instantiated component — the steps remaining are out-of-sample replication, multi-domain calibration, and outcome-validated estimation against eventual startup performance.
...
A note on what this paper and the broader trilogy do and do not establish. This paper provides simulation evidence that, *under the model's assumed candidate-quality distribution and evaluator-parameter ranges*, platform architectures dominate single-stage architectures across a wide range of $\sigma_{\mathrm{AI}}$ and parameter sweeps. It does not provide outcome-validated evidence that platform-enabled angels outperform elite venture firms on realized startup success. Together with Paper 1's modeling framework and Paper 3's in-sample AI-filter proof of concept, the trilogy demonstrates internal coherence and feasibility; out-of-sample replication, multi-domain calibration, and outcome-validated estimation against eventual startup performance are the next study, not this one. We demonstrate that the architecture can dominate under the model; we do not yet claim the model has been outcome-validated at scale.
